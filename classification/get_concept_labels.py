...
if __name__ == "__main__":
    # Set up logging
    logging.basicConfig(
        filename='log_concept_generation.log',
        level=logging.INFO,
    ...
    logging.info(f"Concept Text Sim Model: {args.concept_text_sim_model}")
...
logging.info(f"Saved train concept labels to: {prefix}concept_labels_train.npy")

if args.dataset == 'SetFit/sst2':
    np.save(prefix + "concept_labels_val.npy", val_similarity)
    logging.info(f"Saved validation concept labels to: {prefix}concept_labels_val.npy")

end = time.time()
logging.info(f"Total concept scoring time: {(end-start)/3600:.2f} hours")